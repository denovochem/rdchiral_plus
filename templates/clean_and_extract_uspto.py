...
parsable = Parallel(n_jobs=-1, verbose=1)(
    delayed(can_parse)(rsmi) for rsmi in uspto["ReactionSmiles"].values
)
...
hexhash = (uspto["ReactionSmiles"] + uspto["PatentNumber"]).apply(
...
templates = Parallel(n_jobs=-1, verbose=4)(
    delayed(extract)(reaction) for reaction in reactions
)
