...
from rdchiral.chiral import atom_chirality_matches, copy_chirality, template_atom_could_have_been_tetra
...
def test_template_atom_could_have_been_tetra_degree_three_without_explicit_h_false_and_strips_tag():
    ...
    assert template_atom_could_have_been_tetra(a, strip_if_spec=True, cache=True) is False
...
def test_template_atom_could_have_been_tetra_degree_three_with_explicit_h_true_and_cached():
    ...
    assert template_atom_could_have_been_tetra(a, strip_if_spec=False, cache=True) is True
    assert a.HasProp("tetra_possible")
...
def test_template_atom_could_have_been_tetra_respects_existing_cache_value():
    ...
    assert template_atom_could_have_been_tetra(a, strip_if_spec=False, cache=True) is False
...
def test_atom_chirality_matches_parity_and_tag_controlled_cases(template_smarts, mol_smiles, expected):
...
def test_copy_chirality_inverts_when_atom_chirality_matches_returns_minus_one():
    a_src = _atom_from_smiles("[C@:1]([F:2])([Cl:3])([Br:4])[I:5]", 1)
    a_new = _atom_from_smiles("[C@:1]([F:3])([Cl:2])([Br:4])[I:5]", 1)
